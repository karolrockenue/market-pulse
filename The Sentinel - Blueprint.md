...
The Kick: The API immediately triggers the worker to start processing (for speed).
...
payload: The list of rates (e.g., [{date: '2025-12-25', rate: 120}]).
...
Calculates rates (Base + Differentials).

INSERT the payload into sentinel_job_queue.

Trigger: Sends an async fetch request to the Worker URL (The "Kick").
...
Fetch: Queries the DB for 'PENDING' jobs (FIFO - Oldest first).

Lock: Sets status to 'PROCESSING' so no other worker touches them.

Execute: Sends the data to Cloudbeds (using the Batch API where possible).

Finish:

If Success: Update job status to 'COMPLETED'.
...
Poll sentinel_notifications every ~30-60 seconds.
...
Update api/adapters/sentinel.adapter.js:

Create a new function postRateBatch(propertyId, ratesArray).
...
Modify api/routes/sentinel.router.js (POST /overrides):

Remove: The loop that calls sentinelAdapter.postRate.

Logic:

Calculate all final rates (Base + Differentials).
...
SELECT * FROM sentinel_job_queue WHERE status = 'PENDING' LIMIT 50.

Loop through jobs.

For each job, call sentinelAdapter.postRateBatch.

Success: UPDATE sentinel_job_queue SET status = 'COMPLETED'.

Failure:

UPDATE sentinel_job_queue SET status = 'FAILED', last_error = error.message.
...
Modify SentinelRateManager.tsx:

Update handleSubmitChanges:

Snapshot: Create a copy of pendingOverrides.

Optimistic Update: Immediately merge these into savedOverrides.

Clear Pending: Set setPendingOverrides({}).

Show Status: Set a new state syncStatus = 'SYNCING'.

API Call: Call POST /overrides.

Handle Result:

If success: Set syncStatus = 'SAVED'.
...
Insert <NotificationBell /> next to the User Profile.