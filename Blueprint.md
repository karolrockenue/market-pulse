...
**Aaryan Capital sectioned property dropdown (added 2026-05-20, commit `4c39e5f`).** The top-left property selector in `web/src/components/AppSidebar.tsx` renders a **sectioned** list for non-admin Aaryan Capital / Shreeji owners: a header "Aaryan Capital SP" (Sanchit Patel's portfolio) followed by its hotels in order, then "Aaryan Capital NP" (Nalin Patel's portfolio) in order. Gate: `hasShreejiAccess && !isAdmin && !propertySearch` — admins keep the flat fleet list, and typing in the search box collapses back to a flat filtered list. Any Aaryan hotel not in either order array falls into an "Other" section so it never disappears. The per-portfolio order arrays `AARYAN_ORDER_SP` / `AARYAN_ORDER_NP` are a **frontend mirror** of `DISPLAY_ORDER_SP` / `DISPLAY_ORDER_NP` in `api/services/shreeji.service.js` (§1.1 of `claude/rockenue/groups/shreeji.md`) — **keep the two in sync** whenever a Shreeji hotel onboards/leaves, or the dropdown and the dashboard will disagree. SP = Sanchit Patel, NP = Nalin Patel. Verified live for Sanchit (`role=owner`) and Urmil (`role=user`), both with all 12 hotels linked.
