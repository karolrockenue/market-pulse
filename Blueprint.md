...
**Static London 2026 export (2026-04-23).** A one-shot full-year snapshot of London events lives at repo root as `predicthq-london-2026.tsv` / `predicthq-london-2026.csv` (37,781 unique events, 30 columns; covers mid-Jan → early-Sep 2026 — Oct–Dec is unavailable on this API tier). Generated by `scripts/fetch-predicthq-london-year.js` (weekly chunks, `limit=50/page` free-tier cap, `within=30km@51.5074,-0.1278`, no category/rank filter). Converter: `scripts/tsv-to-csv.js`. Intended as a candidate import into the system so the Demand Radar event layer can be decoupled from the live API when the subscription lapses. Import strategy (filter to `local_rank >= 75` and commit as JSON, vs. bulk load into a new `predicthq_events_raw` table) is undecided — see `claude/todo.md` for the open questions.
