...
// Shared data-assembly for Performance Metrics exports (PDF + XLSX).
// Runs the same getMetricsFromDB + getCompetitorMetrics pipeline the
// on-screen report uses, merged by period with tax-aware aliases.
async function buildPerformanceMetricsPayload({
  propertyId,
  startDate,
  endDate,
  granularity,
  ...
  return { propertyName, rows, metricArr };
}
...
router.post("/reports/performance-metrics/pdf", requireUserApi, async (req, res) => {
  try {
    ...
    const { propertyName, rows, metricArr } = await buildPerformanceMetricsPayload({
      propertyId,
      startDate,
      endDate,
      granularity,
...
router.post("/reports/performance-metrics/xlsx", requireUserApi, async (req, res) => {
  try {
    const {
      propertyId,
      startDate,
    ...
    const { xlsxBuffer, fileName } = await generatePerformanceMetricsXlsx({
      startDate,
      endDate,
      granularity,
      selectedMetrics: metricArr,
    ...
    console.error("Error generating performance-metrics XLSX:", error);
