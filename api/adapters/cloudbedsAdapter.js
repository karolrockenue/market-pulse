 ...
 * @returns {Promise<Object>} - An object summarizing takings by payment method, e.g., { "Cash": 500, "Credit Card": 1250 }.
 */
async function getDailyTakings(accessToken, propertyId, date) {
  // Define the structure of the API request for the Insights API.
  const initialInsightsPayload = {
    property_ids: [propertyId],
    dataset_id: 1, // The "Financial" dataset you discovered.
    filters: {
      and: [
        // Filter 1: Only include transactions for the specified service date.
        {
    ...
    // Specify the columns we need: the amount and the payment method.
    columns: [{ cdf: { column: "credit_amount" }, metrics: ["sum"] }],
    // Group the results by payment method to get totals for each type.
  ...
  console.log(
    `--- STARTING DAILY TAKINGS SYNC for property ${propertyId} on ${date} ---`
  );

  // Loop to handle API pagination, ensuring all payments for the day are fetched.
  do {
    const insightsPayload = { ...initialInsightsPayload };
    if (nextToken) {
      insightsPayload.nextToken = nextToken;
    ...
    if (!apiResponse.ok) {
      console.error(`[Takings Sync] API Error on page ${pageNum}:`, pageData);
      throw new Error(
        `Takings API Error on page ${pageNum}: ${apiResponse.status}`
      );
    }
  ...
  // Process the raw API data into a simple summary object.
  const takingsSummary = {};
  for (const page of allApiData) {
    if (!page.index || !page.records) continue;
    for (let i = 0; i < page.index.length; i++) {
      const paymentMethod = page.index[i][0] || "Unknown";
      const totalAmount = parseFloat(page.records.credit_amount?.[i]) || 0;
      if (totalAmount > 0) {
        takingsSummary[paymentMethod] = totalAmount;
      }
    }
  }

  console.log("--- DAILY TAKINGS SYNC COMPLETE ---");
