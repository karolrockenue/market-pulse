...
// --- INITIAL LOAD ---
// --- INITIAL LOAD ---
// Set the initial date range based on the default preset and generate the report
...
function saveSchedule() {
  const name = document.getElementById("schedule-name").value || "Untitled";
  const tbody = document.getElementById("scheduled-reports-tbody");
  const newRow = document.createElement("tr");
  newRow.innerHTML = `<td class="px-6 py-4"><div class="text-sm font-semibold">${name}</div></td><td></td><td><a href="#" class="text-red-600 hover:text-red-900 delete-schedule-btn">Delete</a></td>`;
  ...
  updateScheduleCount();
  // This function call will no longer work, and that's okay for now.
  // We will fix this when we refactor the save functionality itself.
  // closeScheduleModal();
}
// ==================================================================
// REPORT GENERATION & RENDERING
// ==================================================================
function handleGenerateReport(granularity) {
...
function generateReportTitle(selected) {
  ...
  if (selected.market.length > 0 && selected.hotel.length > 0) {
    title += " vs The Market";
  }
  return title;
}
...
function buildTableHeaders(selected, addComparisons, displayOrder) {
  ...
  if (!addComparisons) {
    // If not in comparison mode, just show all selected columns in order.
    ...
    // Comparison mode logic (which was already working correctly)
