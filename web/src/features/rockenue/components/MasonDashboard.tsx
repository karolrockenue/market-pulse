...
function RecentBookingsMock() {
  // Newest first — today at top
  const rows = useMemo(() => {
    ...
    for (let i = 0; i <= 6; i++) {
      ...
      const bookings = wknd ? 14 + Math.round(Math.random() * 4) : 8 + Math.round(Math.random() * 4);
      const nights = bookings * (wknd ? 3.2 : 2.4);
      const adr = wknd ? 198 : 176;
      const revenue = nights * adr;
      out.push({
        dateStr: d.toLocaleDateString("en-GB", { weekday: "short", day: "numeric", month: "short" }),
        bookings,
        roomNights: Math.round(nights),
        adr,
        revenue,
        isToday: i === 0,
      });
  ...
  return (
    <div style={{ padding: 0, height: "100%", display: "flex", flexDirection: "column" }}>
      ...
      >
        ...
        >
          <Calendar style={{ width: 16, height: 16, color: "#38C6BA" }} />
        </div>
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          >
            ...
            <span
              title="Scoped to the Short Stay Accommodation service"
              style={{
                display: "inline-flex",
                alignItems: "center",
                gap: 4,
                padding: "2px 8px",
                borderRadius: 4,
                background: "rgba(123,175,212,0.10)",
          ...
          <div style={{ color: R.textDim, fontSize: 10, marginTop: 3, letterSpacing: 0.2 }}>
      ...
      >
        <div style={{ color: R.textDim, fontSize: 9, textTransform: "uppercase" }}>Date</div>
        <div style={{ color: R.textDim, fontSize: 9, textTransform: "uppercase", textAlign: "right" }}>
          Bookings
        </div>
        <div style={{ color: R.textDim, fontSize: 9, textTransform: "uppercase", textAlign: "right" }}>
          Room Nights
        ...
        {rows.map((day, i) => (
          <div
            ...
            style={{
              ...
              borderRadius: 4,
              margin: "2px 6px",
              backgroundColor: day.isToday ? "rgba(123,175,212,0.04)" : "transparent",
              border: day.isToday ? "1px solid rgba(123,175,212,0.10)" : "1px solid transparent",
            }}
          >
            <div style={{ color: day.isToday ? "#7BAFD4" : R.accent, fontSize: 11 }}>
              {day.dateStr}
            </div>
            <div style={{ color: R.accent, fontSize: 11, textAlign: "right" }}>{day.bookings}</div>
            <div style={{ color: R.accent, fontSize: 11, textAlign: "right" }}>{day.roomNights}</div>
            <div style={{ color: R.accent, fontSize: 11, textAlign: "right" }}>£{formatCompact(day.adr)}</div>
            <div
              style={{
                color: "#7BAFD4",
                fontSize: 11,
    ...
    >
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 16 }}>
        <SectionHeader
          ...
          subtitle={
            data
              ? `${data.tyYear} vs ${data.lyYear} · per service × month · YTD through month ${data.actualMonths} · ${vatLabel}`
              : loading
  ...
  return (
    <div style={{ minWidth: 1040 }}>
      ...
      {SERVICES.map((svc, sidx) => {
        const ty = byService[svc.key].ty;
        const ly = byService[svc.key].ly;
        const ytdTy = ty.slice(0, actualMonths).reduce((a, b) => a + b, 0);
        const ytdLy = ly.slice(0, actualMonths).reduce((a, b) => a + b, 0);
        const ytdDelta = deltaPct(ytdTy, ytdLy);

        return (
          <div key={svc.key} style={{ paddingTop: sidx > 0 ? 12 : 8 }}>
            {/* Service label */}
            <div
              style={{
                display: "flex",
            ...
            >
              <span style={{ width: 10, height: 10, borderRadius: 3, background: svc.color }} />
              <span
                style={{
                  fontSize: 12,
                  color: R.accent,
            ...
            >
              ...
              <div style={{ fontSize: 12, color: R.accent, fontWeight: 700, textAlign: "right", padding: "0 6px" }}>
                {fmtK(ytdTy)}
              </div>
            </div>
      ...
      {/* Grand total block */}
      <div style={{ marginTop: 14, padding: "12px 0", background: "rgba(56,198,186,0.04)", borderRadius: 6 }}>
        <div
          style={{
            display: "grid",
            gridTemplateColumns: "150px repeat(12, 1fr) 90px",
        ...
        >
          ...
          <div style={{ textAlign: "right", padding: "0 6px" }}>
            <DeltaChip pct={deltaPct(ytdTotTy, ytdTotLy)} size="sm" />
          </div>
        </div>
      </div>
    </div>
...
function YoYSkeleton() {
  return (
    <div style={{ minWidth: 1040 }}>
      ...
      {SERVICES.map((svc) => (
        <div key={svc.key} style={{ padding: "10px 0", borderBottom: `1px solid ${R.sep}` }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
            <span style={{ width: 10, height: 10, borderRadius: 3, background: svc.color, opacity: 0.45 }} />
...
export function MasonDashboard({ scopedHotelId = null }: MasonDashboardProps = {}) {
