...
export const generateReport = async (
  ...
  const fetchHotelMetrics =
    metrics.length === 0 || metrics.some((m) => !m.startsWith("market-"));
  const fetchMarketMetrics =
    metrics.length === 0 || metrics.some((m) => m.startsWith("market-"));

  // 2. Define Tax Aliases (Logic from OldReportTable)
  const hotelAdrAlias = includeTaxes ? "your_gross_adr" : "your_net_adr";
  const hotelRevparAlias = includeTaxes
    ? "your_gross_revpar"
    : "your_net_revpar";
  const hotelRevenueAlias = includeTaxes
    ? "your_gross_revenue"
    : "your_net_revenue";

  const marketAdrAlias = includeTaxes ? "market_gross_adr" : "market_net_adr";
  const marketRevparAlias = includeTaxes
    ? "market_gross_revpar"
    : "market_net_revpar";
  const marketRevenueAlias = includeTaxes
    ? "market_gross_revenue"
    : "market_net_revenue";
  ...
  try {
    ...
    if (marketData.metrics) {
      marketData.metrics.forEach((row: any) => {
        ...
        entry["market-total-revenue"] =
          parseFloat(row[marketRevenueAlias]) || 0;
      });
    }
...
export const fetchTakingsReport = async (
  startDate: string,
  endDate: string,
  hotelId?: string
): Promise<any[]> => {
  const response = await axios.get(`${BASE_URL}/reports/takings`, {
    params: { startDate, endDate, propertyId: hotelId },
  });
  return response.data;
};
