...
import { Calendar, Copy, Zap, Download, Loader2, Percent } from "lucide-react";
...
export function DailyMaxRatesDialog({
  ...
  const handleApplyModifier = (monthId: number, percentage: number) => {
    const days = getDaysInMonth(monthId);
    setRates((prev) => {
      const next = { ...prev };
      ...
      days.forEach((day) => {
        ...
        const currentVal = parseFloat(next[key]);
        if (!isNaN(currentVal)) {
          const newVal = Math.round(currentVal * (1 + percentage / 100));
          next[key] = newVal.toString();
          hasChanges = true;
        }
      });
